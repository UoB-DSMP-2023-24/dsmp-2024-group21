...
def render_charts(portfolio, tapes_df):
    ...
    def create_chart(title, y_data):
        ...
        chart.add_yaxis(
            series_name=title,
            y_axis=y_data,
            label_opts=opts.LabelOpts(is_show=False),  # 隐藏坐标点值的显示
            is_smooth=True
        )   
        chart.set_global_opts(
            ...
            yaxis_opts=opts.AxisOpts(is_scale=True),  # 自动缩放坐标范围
...
def backend(lob_hdf5, tapes_hdf5, date):
    ...
    risk_per_trade = 0.04
    tapes_df['SMA10'] = tapes_df['price'].rolling(window=10).mean()
    tapes_df['SMA50'] = tapes_df['price'].rolling(window=50).mean()
